...
const { render, newPages } = await import(pathToFileURL(path.join(ssr, 'entry-server.js')).href);
...
const pages = [
  ...['index.html', ...routes].map((file) => ({
    ...
    head: extract(file).metadata.replace(/https:\/\/www\.silicomindia\.com\/assets\/silicom-logo-full\.png/g, 'https://www.silicomindia.com/brand/og-image.png'),
  })),
...
fs.writeFileSync(path.join(dist, 'sitemap.xml'), fs.readFileSync(path.join(root, 'sitemap.xml'), 'utf8').replace('</urlset>', `${entries}\n</urlset>`));
